...
from market_plugins.galgame_plugin.store import GalgameStore
...
def test_galgame_config_overrides_apply_valid_values_and_ignore_invalid(tmp_path: Path) -> None:
    ...
    assert plugin._cfg.ocr_reader.ocr_reader_screen_templates == [{"id": "title", "stage": "title_stage"}]
...
def test_galgame_store_context_snapshot_strict_load_rejects_empty_game_id_and_expires(
    ...
    store.persist_context_snapshot({"game_id": "", "summary_seed": "no game", "saved_at": time.time()})
    assert store.load_context_snapshot(current_game_id="") == {}
    assert store.load_context_snapshot(current_game_id="", require_game_id=False)["summary_seed"] == "no game"
