...
import pytest
from market_plugins.galgame_plugin import llm_backend as galgame_llm_backend
from market_plugins.galgame_plugin.llm_backend import GalgameLLMBackend
...
def test_llm_backend_prompt_message_contracts_are_stable() -> None:
    ...
    payload = {operation: backend._build_messages(operation, context) for operation, context in contexts.items()}
...
async def test_llm_backend_create_chat_llm_does_not_receive_temperature(
    ...
    async def _fake_create_chat_llm_async(**kwargs):
        captured.update(kwargs)
        return _FakeLLM()
...
def test_galgame_plugin_toml_defaults_to_internal_backend() -> None:
    path = Path(__file__).resolve().parents[2] / "plugin.toml"
