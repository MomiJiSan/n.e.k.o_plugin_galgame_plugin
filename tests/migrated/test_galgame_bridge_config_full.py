...
import pytest
from market_plugins.galgame_plugin import plugin_core as galgame_plugin_module
from market_plugins.galgame_plugin import reader as galgame_reader
...
from tests.support.ocr_flow import _ocr_reader_session
from tests.support.scene_capsule import _shared_state

market_package = import_module("market_plugins.galgame_plugin")
market_package.build_summarize_context = build_summarize_context
...
def test_expand_bridge_root_handles_user_home_defaults(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(galgame_reader.os.path, "expanduser", _fake_expanduser)
...
async def test_background_bridge_poll_exception_records_error(tmp_path: Path) -> None:
    ...
    assert task is not None
    await asyncio.wait_for(task, timeout=0.5)
    plugin._clear_completed_background_bridge_poll(task)
