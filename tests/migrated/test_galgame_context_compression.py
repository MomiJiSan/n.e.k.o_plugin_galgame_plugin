...
import pytest
from market_plugins.galgame_plugin.context_builder import build_fallback_summary
from market_plugins.galgame_plugin.layered_memory import LayeredMemory
from market_plugins.galgame_plugin.push_composer import (
    MAX_PUSH_TOKENS,
    PLOT_MIN_TOKENS,
    PushComposer,
)
...
def _basic_plot_payload(seq: int = 1, key_line_count: int = 2) -> dict[str, Any]:
    return {
        ...
        "plot_update": {
            ...
            "key_lines": [
                {"speaker": "叢雨", "text": f"わらわは別に気にしてなどおらぬ_{i}"} for i in range(key_line_count)
            ],
            "new_choices": [
...
def test_compose_returns_self_contained_payload() -> None:
    ...
    assert any(q["entry"] == "galgame_get_scene_context" for q in result["available_queries"])
...
def test_layered_memory_extend_lines_atomic() -> None:
    ...
    mem.extend_lines([{"text": "a"}, "garbage", {"text": "b"}, {"text": "c"}])
...
async def test_update_story_so_far_swaps_atomically() -> None:
    ...
    class _Summarizer:
        ...
        async def summarize_story(self, *, current_story: str, new_scenes: list[str]) -> str:
