...
from market_plugins.galgame_plugin.plugin_config_service import (
...
def test_config_service_persists_runtime_overrides_to_store() -> None:
    ...
    service = GalgamePluginConfigService(
        SimpleNamespace(
            _persist=SimpleNamespace(persist_config_override=lambda key, value: writes.append((key, value)))
        )
    )
...
def test_json_copy_preserves_copy_semantics() -> None:
...
def test_galgame_config_groups_fields_and_keeps_flat_compatibility(
    tmp_path: Path,
) -> None:
    cfg = GalgameConfig(
