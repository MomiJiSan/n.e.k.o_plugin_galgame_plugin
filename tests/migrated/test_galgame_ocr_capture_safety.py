...
_helpers = import_module(
    "market_plugins.galgame_plugin.ocr_capture_backends._helpers"
)
dxcam_backend = import_module(
    "market_plugins.galgame_plugin.ocr_capture_backends.dxcam"
)
mss_backend = import_module(
    "market_plugins.galgame_plugin.ocr_capture_backends.mss"
)
pyautogui_backend = import_module(
    "market_plugins.galgame_plugin.ocr_capture_backends.pyautogui"
)
win32_backend = import_module(
    "market_plugins.galgame_plugin.ocr_capture_backends.win32"
)
...
OcrCaptureProfile = ocr_reader_module.OcrCaptureProfile
Win32CaptureBackend = ocr_reader_module.Win32CaptureBackend
...
def test_plugin_default_capture_backend_is_smart() -> None:
    ...
    config = tomllib.loads(
        (repo_root / "plugin.toml").read_text(encoding="utf-8")
    )
