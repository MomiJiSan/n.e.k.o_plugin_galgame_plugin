...
def test_local_input_visible_choice_detection_accepts_menu_flag_or_choices() -> None:
    assert local_input._snapshot_has_visible_choices({"latest_snapshot": {"is_menu_open": True, "choices": []}}) is True
    assert (
        local_input._snapshot_has_visible_choices(
            {"latest_snapshot": {"is_menu_open": False, "choices": [{"text": "Left"}]}}
        )
        is True
    )
    assert (
        local_input._snapshot_has_visible_choices({"latest_snapshot": {"is_menu_open": False, "choices": []}}) is False
    )


def test_local_input_choose_index_uses_choice_payload_index() -> None:
    assert (
        local_input._choose_index(
            {
                "candidate_index": 0,
                "candidate_choices": [
