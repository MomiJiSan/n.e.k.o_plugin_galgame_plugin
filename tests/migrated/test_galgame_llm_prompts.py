...
from market_plugins.galgame_plugin import llm_prompts
from market_plugins.galgame_plugin.agent_scene_context import AgentSceneContextMixin
from market_plugins.galgame_plugin.context_builder import build_local_scene_summary
from market_plugins.galgame_plugin.llm_prompts import (
...
def test_prompt_context_uses_default_token_budget() -> None:
    ...
    configured_rendered = _rendered_context(
        build_prompt_messages_with_metadata(
            "agent_reply",
            context,
...
def test_scene_delta_capsule_only_exposes_latest_confirmed_target() -> None:
    ...
    continuity = content.split("连续性背景（最多 2 条，不是回应对象）：", 1)[1].split("本次回应对象：", 1)[0]
...
def test_scene_delta_capsule_preserves_complete_visible_choice_group() -> None:
    choices = [{"text": f"Option {index}", "choice_state": "visible"} for index in range(1, 6)]
...
def test_scene_delta_capsule_rejects_tentative_only_delta() -> None:
    content = AgentSceneContextMixin._format_scene_delta_for_cat(
        new_stable_lines=[{"speaker": "OCR", "text": "not confirmed", "stability": "tentative"}],
        new_choices=[],
        continuity_lines=[{"speaker": "A", "text": "old context", "stability": "stable"}],
    )
...
def test_scene_delta_capsule_preserves_legitimate_repeated_line_event() -> None:
    content = AgentSceneContextMixin._format_scene_delta_for_cat(
        new_stable_lines=[{"seq": 2, "speaker": "A", "text": "same words", "stability": "stable"}],
        new_choices=[],
        continuity_lines=[{"seq": 1, "speaker": "A", "text": "same words", "stability": "stable"}],
    )
