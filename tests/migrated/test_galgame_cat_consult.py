...
import pytest
from market_plugins.galgame_plugin.agent_consultation import (
...
def test_mode_off_never_consults() -> None:
    decision = decide_consultation(ConsultInputs(character_mode="off", character_fixed_name="叢雨"))
...
def test_scene_change_trigger_when_no_choices(
    ...
    decision = decide_consultation(ConsultInputs(**{**asdict(fixed_mode_base), "scene_changed": True}))
    assert decision.should_consult is True
    ...
    decision = decide_consultation(
        ConsultInputs(
            **{
                **asdict(fixed_mode_base),
                "visible_choices": ("a", "b"),
                "last_consult_ts": fixed_mode_base.now - (CONSULT_COOLDOWN_SECONDS + 1.0),
            }
        )
    )
    assert decision.should_consult is True
