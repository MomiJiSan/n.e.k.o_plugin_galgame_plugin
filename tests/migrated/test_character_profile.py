...
import pytest
from market_plugins.galgame_plugin.character_profile import (
    L0_MAX_TOKENS,
    L1_MAX_TOKENS,
    L2_MAX_TOKENS,
...
VALID_PRESET: dict[str, object] = {
    ...
    "characters": {
        "雪乃": {
            ...
            "key_events": [{"event": "主角邀请放学一起回家", "significance": "关系起点"}],
        },
        "结衣": {
            "identity": "活泼开朗的同班同学",
            "character_voice": {
                "core_traits": [{"trait": "外向热情", "speech_effect": "句尾常带！，语速快"}],
                "verbal_tics": ["やっはろー"],
...
def test_load_game_profiles_preset_only(manager: CharacterProfileManager, tmp_path: Path) -> None:
...
def test_load_game_profiles_user_overrides_preset(manager: CharacterProfileManager, tmp_path: Path) -> None:
    _write(tmp_path / "demo_game.json", VALID_PRESET)
    user_payload = {
        "game_id": "demo_game",
        ...
        "characters": {
            "雪乃": {
                ...
                "character_voice": {
                    "core_traits": [{"trait": "改造性格", "speech_effect": "改造说话方式"}],
                    "verbal_tics": ["新口癖"],
            ...
            "新角色": {
                "identity": "用户添加的角色",
                "character_voice": {"core_traits": [{"trait": "用户性格", "speech_effect": "用户语调"}]},
            },
        },
    }
...
def test_load_game_profiles_user_only_without_preset(manager: CharacterProfileManager, tmp_path: Path) -> None:
    user_payload = {
        "game_id": "demo_game",
        "last_updated": "2026-05-20",
        "characters": {
            "雪乃": {
                ...
                "character_voice": {"core_traits": [{"trait": "自定义性格", "speech_effect": "自定义语调"}]},
            }
        },
    }
    _write(tmp_path / "demo_game.user.json", user_payload)
...
def test_load_game_profiles_broken_user_falls_back_to_preset(manager: CharacterProfileManager, tmp_path: Path) -> None:
    _write(tmp_path / "demo_game.json", VALID_PRESET)
    ...
    user_payload = {
        ...
        "characters": {
            "自定义角色": {
                "identity": "用户导入的自定义角色",
                "character_voice": {
                    "core_traits": [{"trait": "谨慎", "speech_effect": "先确认事实再回答"}],
                },
            },
        },
    }
...
def test_load_game_profiles_oversize_rejected(manager: CharacterProfileManager, tmp_path: Path) -> None:
...
def test_load_game_profiles_cached_on_second_call(manager: CharacterProfileManager, tmp_path: Path) -> None:
    _write(tmp_path / "demo_game.json", VALID_PRESET)
...
def test_resolve_profile_match_prefers_exact_game_id(manager: CharacterProfileManager, tmp_path: Path) -> None:
    _write(tmp_path / "demo_game.json", VALID_PRESET)
...
def test_resolve_profile_match_uses_alias_and_window_title(manager: CharacterProfileManager, tmp_path: Path) -> None:
    payload = {
        **VALID_PRESET,
        "aliases": ["千恋万花"],
        "match": {
    ...
    window_match = manager.resolve_profile_match([{"window_title": "千恋＊万花 - Steam"}])
    process_match = manager.resolve_profile_match([{"process_name": "SenrenBanka.exe"}])
...
def test_resolve_profile_match_includes_user_only_profile(manager: CharacterProfileManager, tmp_path: Path) -> None:
    _write(
...
def test_missing_required_field_is_error(manager: CharacterProfileManager, tmp_path: Path) -> None:
    payload = {
        "game_id": "demo",
        "characters": {
            "破角色": {
                # missing "identity"
                "character_voice": {"core_traits": [{"trait": "x", "speech_effect": "y"}]}
            }
        },
    }
...
def test_core_trait_missing_speech_effect_is_error(manager: CharacterProfileManager, tmp_path: Path) -> None:
    payload = {
        "game_id": "demo",
        "characters": {
            "角色": {
    ...
    _write(tmp_path / "demo.json", payload)
    result = manager.load_game_profiles("demo")
    assert any("speech_effect" in err and "不能为空" in err for err in result["errors"])


def test_user_json_soft_consistency_warning(manager: CharacterProfileManager, tmp_path: Path) -> None:
    ...
    user_payload = {
        ...
        "characters": {
            "雪乃": {
                "identity": "user",
                "character_voice": {"core_traits": [{"trait": "暴躁", "speech_effect": "轻声细语"}]},
            }
        },
    }
...
def test_get_profiles_for_scene_filters_to_named_characters() -> None:
    ...
    out = CharacterProfileManager.get_profiles_for_scene(all_profiles, ["雪乃", "雪乃", "", "结衣", "缺失角色"])
...
def test_build_character_push_payload_unknown_level_defaults_to_l1() -> None:
    manager = CharacterProfileManager(data_dir=CHARACTER_DATA_DIR)
    profiles = {"叢雨": _murasame_profile()}
    payload = manager.build_character_push_payload(["叢雨"], profiles, runtime_states={}, level="weird")
...
def test_init_runtime_state_minor_character() -> None:
    ...
    minor = {
        "identity": "minor",
        "character_voice": {"core_traits": [{"trait": "background", "speech_effect": "few lines"}]},
    }
...
def test_import_user_profiles_writes_user_json(manager: CharacterProfileManager, tmp_path: Path) -> None:
    ...
    user_payload = {
        ...
        "characters": {
            "雪乃": {
                "identity": "imported",
                "character_voice": {"core_traits": [{"trait": "import 性格", "speech_effect": "import 语调"}]},
            }
        },
    }
...
def test_import_user_profiles_rejects_invalid_payload(manager: CharacterProfileManager, tmp_path: Path) -> None:
