...
import pytest
from market_plugins.galgame_plugin import context_builder as galgame_context_builder
from market_plugins.galgame_plugin import service as galgame_service
...
from market_plugins.galgame_plugin.service import (
...
def test_build_config_defaults_vision_classifier_model_dir_inside_plugin() -> None:
    ...
    assert cfg.vision_classifier_model_dir == "models/vision/screen_classifier"
...
def test_build_config_reads_context_optimization_fields() -> None:
    ...
    low_threshold = galgame_service.build_config({"llm": {"llm_repeat_similarity_threshold": -1}})
...
def test_stable_line_promotes_matching_observed_entry_out_of_tentative_history() -> None:
    ...
    _history_events, stable, observed, _choices, _dedupe, _snapshot = galgame_service.rebuild_histories_from_events(
        events=events,
        snapshot={},
        dedupe_window=[],
        config=galgame_service.build_config({}),
...
def test_stable_line_only_promotes_observed_entry_from_matching_scene() -> None:
    ...
    assert [(item["scene_id"], item["line_id"]) for item in stable] == [("scene-b", "ocr:same-text-hash")]
    assert [(item["scene_id"], item["line_id"]) for item in observed] == [("scene-a", "ocr:same-text-hash")]
...
def test_stable_history_keeps_repeated_text_in_each_scene_after_snapshot_advances() -> None:
    ...
    history_events, stable, observed, choices, dedupe, snapshot = galgame_service.rebuild_histories_from_events(
        events=events,
        snapshot={},
        dedupe_window=[],
        config=galgame_service.build_config({}),
...
def test_status_payload_exposes_developer_managed_dialogue_library_status(
    ...
    active_pack = next(pack for pack in library_status["packs"] if pack["game_id"] == library_status["active_game_id"])
...
def test_status_payload_primary_diagnosis_reports_capture_failure(
    ...
    assert {"recalibrate_ocr", "capture_backend", "debug_details"} <= {action["id"] for action in diagnosis["actions"]}
