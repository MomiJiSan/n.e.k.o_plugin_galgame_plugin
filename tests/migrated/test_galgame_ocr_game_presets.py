...
from tests.support.node_harness import run_node_script
...
def test_senren_banka_frontend_profile_matches_backend_preset() -> None:
    ...
    source = (Path(__file__).resolve().parents[2] / "static" / "main.js").read_text(encoding="utf-8")
    constants = (
        source[source.index("const DEFAULT_CAPTURE_PROFILE") : source.index("const OCR_PROFILE_STAGE_LABELS_ZH")]
        + source[source.index("const AIHONG_CAPTURE_PRESETS") : source.index("const AUTO_REFRESH_IDLE_INTERVAL_MS")]
    )
    helpers = source[source.index("function normalizeProcessName") : source.index("function setInputValueIfIdle")]
    script = f"""
...
def test_senren_banka_frontend_profile_can_be_selected_manually() -> None:
    ...
    helpers = source[source.index("function normalizeProcessName") : source.index("function setInputValueIfIdle")]
    selector = source[source.index("function renderOcrProfile") : source.index("function renderHistory")]
    render_helpers = source[
        source.index("function setInputValueIfIdle") : source.index("function renderInstallTaskState")
    ]
    script = f"""
const assert = require('node:assert/strict');
const vm = require('node:vm');
...
def test_saving_selected_game_preset_returns_editor_to_stored_profile() -> None:
    ...
    save_functions = source[
        source.index("function readProfileNumber") : source.index("async function clearOcrCaptureProfile")
    ]
    script = f"""
const assert = require('node:assert/strict');
const vm = require('node:vm');
