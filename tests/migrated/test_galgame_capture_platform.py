...
cp = import_module("market_plugins.galgame_plugin.capture_platform")
...
def test_linux_window_scanner_returns_list() -> None:
    from market_plugins.galgame_plugin.window_scanner_linux import (
        _scan_windows_linux,
    )
...
def test_macos_window_scanner_returns_list() -> None:
    from market_plugins.galgame_plugin.window_scanner_macos import (
        _scan_windows_macos,
    )
...
def test_macos_target_window_rect_prefers_window_number_over_pid(
    ...
    from market_plugins.galgame_plugin import ocr_reader
...
def test_linux_xlib_scanner_marks_hidden_windows_minimized(
    ...
    from market_plugins.galgame_plugin import window_scanner_linux
...
def test_electron_backend_is_available_when_endpoint_down(
    ...
    import httpx
    from market_plugins.galgame_plugin.electron_capture import (
...
def test_electron_backend_resolves_default_url_from_env(
    ...
    from market_plugins.galgame_plugin import electron_capture
...
def test_electron_backend_applies_capture_profile_crop(
    ...
    from market_plugins.galgame_plugin.ocr_reader import OcrCaptureProfile
...
def test_build_backends_filters_win32_only_on_non_windows(is_linux_host: bool) -> None:
    ...
    from market_plugins.galgame_plugin import ocr_reader as ocr_reader_mod
    from market_plugins.galgame_plugin.ocr_reader import Win32CaptureBackend

    with patch.object(ocr_reader_mod, "_is_windows_platform", return_value=False), patch(
        "market_plugins.galgame_plugin.capture_platform.is_windows",
        return_value=False,
    ), patch(
        "market_plugins.galgame_plugin.capture_platform.is_linux",
