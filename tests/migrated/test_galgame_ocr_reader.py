...
import pytest
from market_plugins.galgame_plugin import dialogue_library as galgame_dialogue_library
from market_plugins.galgame_plugin import ocr_backends as galgame_ocr_backends
from market_plugins.galgame_plugin import ocr_bridge_writer as galgame_ocr_bridge_writer
from market_plugins.galgame_plugin import ocr_capture as galgame_ocr_capture
from market_plugins.galgame_plugin import ocr_capture_backends as galgame_ocr_capture_backends
from market_plugins.galgame_plugin import ocr_manager_text as galgame_ocr_manager_text
from market_plugins.galgame_plugin import ocr_rapidocr_backend as galgame_ocr_rapidocr_backend
from market_plugins.galgame_plugin import ocr_reader as galgame_ocr_reader
from market_plugins.galgame_plugin import ocr_text_normalize as galgame_ocr_text_normalize
from market_plugins.galgame_plugin.models import (
...
from market_plugins.galgame_plugin.ocr_capture_backends import _helpers as galgame_ocr_capture_helpers
from market_plugins.galgame_plugin.ocr_capture_backends import dxcam as galgame_dxcam_backend
from market_plugins.galgame_plugin.ocr_capture_backends import printwindow as galgame_printwindow_backend
from market_plugins.galgame_plugin.ocr_capture_backends import pyautogui as galgame_pyautogui_backend
from market_plugins.galgame_plugin.ocr_manager_observe import SceneObservation
from market_plugins.galgame_plugin.ocr_reader import (
    DetectedGameWindow,
...
from market_plugins.galgame_plugin.reader import read_session_json, tail_events_jsonl
from market_plugins.galgame_plugin.screen_awareness_training import (
    evaluate_screen_awareness_model,
    train_screen_awareness_model,
)
from market_plugins.galgame_plugin.screen_classifier import (
...
def test_screen_classifier_filters_window_chrome_noise_from_regions() -> None:
    ...
    assert [element["text"] for element in classified.ui_elements] == ["有人是猪马牛羊，有人是虎豹豺狼。"]
...
def test_ocr_reader_known_title_timeout_triggers_rescan_and_skip_bypass(
    ...
    assert title_classification.debug["skip_dialogue_bypass_reason"] == ("known_screen_timeout_rescan")
...
def test_ocr_reader_title_classification_only_bypasses_for_strong_dialogue_text(
    ...
    assert (
        manager._should_skip_dialogue_for_screen_classification(
            classification,
...
def test_ocr_reader_cnn_title_narration_requires_repeat_and_timeout(
    ...
    assert classification.debug["skip_dialogue_bypass_reason"] == ("stable_title_narration_timeout_rescan")
...
def test_cnn_dialogue_boundary_requires_known_nonempty_label(tmp_path: Path) -> None:
    ...
    assert manager._reconcile_screen_classification_with_active_dialogue(classification) is classification
...
def test_active_dialogue_does_not_override_fresh_cnn_choice_menu(tmp_path: Path) -> None:
    ...
    reconciled = manager._reconcile_screen_classification_with_active_dialogue(classification)
...
async def test_ocr_reader_cnn_title_false_positive_preserves_dialogue_stability(
    ...
    try:
        ...
        assert first_events[-1]["payload"]["screen_debug"]["source"] == ("ocr_dialogue_reconciliation")
        assert first_events[-1]["payload"]["screen_debug"]["original_screen_debug"][
            "model_confidence_raw"
        ] == pytest.approx(0.97)
        assert first.runtime["last_observed_line"]["text"] == ("那当然了。也没有感冒，腰杆笔直，腿脚利落")
        ...
        assert session is not None
        assert session["state"]["screen_type"] == OCR_CAPTURE_PROFILE_STAGE_DIALOGUE
        assert second.runtime["last_stable_line"]["text"] == ("那当然了。也没有感冒，腰杆笔直，腿脚利落")
    finally:
...
async def test_ocr_reader_cnn_title_narration_waits_for_timeout_then_stays_dialogue(
    ...
    try:
        ...
        events_path = bridge_root / first.runtime["game_id"] / "events.jsonl"
        assert all(event["type"] not in {"line_observed", "line_changed"} for event in _read_events(events_path))
        ...
        assert fourth_events[-1]["payload"]["screen_type"] == (OCR_CAPTURE_PROFILE_STAGE_DIALOGUE)
        ...
        assert session is not None
        assert session["state"]["screen_type"] == OCR_CAPTURE_PROFILE_STAGE_DIALOGUE
        assert session["state"]["screen_debug"]["source"] == ("ocr_dialogue_reconciliation")
    finally:
        await manager.shutdown()
...
async def test_ocr_reader_cnn_choice_menu_false_positive_keeps_narration_dialogue(
    ...
    try:
        ...
        assert second_events[-1]["payload"]["screen_type"] == (OCR_CAPTURE_PROFILE_STAGE_DIALOGUE)
...
def test_ocr_reader_line_reconciles_non_cnn_menu_false_positive(
    ...
    assert events[-1]["payload"]["screen_type"] == OCR_CAPTURE_PROFILE_STAGE_DIALOGUE
    assert events[-1]["payload"]["screen_debug"]["original_screen_debug"] == {"reason": "visual_button_layout"}
    assert manager._dialogue_pipeline.state.effective_screen_type == (OCR_CAPTURE_PROFILE_STAGE_DIALOGUE)
...
def test_ocr_session_snapshot_write_failure_is_nonfatal(
    ...
    window = _window()[0]
    expected_game_id = galgame_ocr_reader._ocr_game_id_from_process(window.process_name or window.title)
...
async def test_ocr_reader_capture_timeout_skips_stuck_worker_immediately(
    ...
    try:
        ...
        assert not any(
            warning[0] == "ocr_reader replacing stuck capture worker after %.1fs timeout" for warning in logger.warnings
        )
        ...
        assert third.runtime["detail"] == "receiving_text", third.runtime
        assert any(
            warning[0] == "ocr_reader rotating timed-out capture executor after {:.1f}s; cancel_requested={}"
...
async def test_ocr_reader_capture_timeout_recovers_cancellable_worker(
    ...
    try:
        ...
        assert backend.calls == 1
        assert result.runtime["detail"] == "receiving_text", result.runtime.get("last_capture_error")
        assert any(
            warning[0] == "ocr_reader rotating timed-out capture executor after {:.1f}s; cancel_requested={}"
            for warning in logger.warnings
        )
...
async def test_ocr_reader_capture_timeout_recovery_is_bounded(
    ...
    try:
        ...
        assert any(
            "capture timed out" in warning and "recovery limit reached" in warning for warning in result.warnings
        )
        with manager._capture_worker_lock:
        ...
        assert backend.calls == 1
        assert retry.runtime["detail"] == "receiving_text", retry.runtime
    finally:
        await manager.shutdown()
...
def test_aihong_menu_choice_parser_accepts_plain_text_without_status_lines() -> None:
    assert galgame_ocr_reader._coerce_aihong_menu_choices(["往南跑", "躲进巷子里"]) == ["往南跑", "躲进巷子里"]


def test_aihong_menu_choice_parser_rejects_unpunctuated_chinese_dialogue() -> None:
    assert galgame_ocr_reader._coerce_aihong_menu_choices(["你今天去哪里", "我还没想好", "不如一起走"]) == []


def test_aihong_menu_choice_parser_keeps_prefixed_choices_with_pronouns() -> None:
    assert galgame_ocr_reader._coerce_aihong_menu_choices(["1. 我选择留下", "2. 你先走吧"]) == [
        "我选择留下",
        "你先走吧",
    ]
...
def test_capture_image_hash_accepts_stable_text_frames() -> None:
    ...
    assert OcrReaderManager._capture_image_hash("frame:1") == OcrReaderManager._capture_image_hash("frame:1")
...
def test_background_hash_excludes_bottom_dialogue_region() -> None:
    ...
    assert cropped_first.info["galgame_source_background_hash"] == cropped_second.info["galgame_source_background_hash"]
...
def test_dxcam_camera_creation_is_serialized(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    threads = [threading.Thread(target=lambda: results.append(backend._camera_instance())) for _ in range(2)]
...
def test_pending_visual_scene_waits_for_stable_line_before_commit(tmp_path: Path) -> None:
    ...
    assert manager._runtime.scene_ordering_diagnostic == ("pending_scene_committed_before_observed")
...
def test_pending_visual_scene_capture_failure_preserves_candidate_for_recovery(
    ...
    assert result.runtime["detail"] == "capture_failed"
    assert result.runtime["last_capture_error"] == ("foreground_changed_during_screen_capture")
    assert manager._pending_visual_scene_hash == "ffffffffffffffff"
...
def test_pending_visual_scene_timeout_discards_boundary_without_empty_scene(
    ...
    assert manager._pending_visual_scene_hash == ""
    assert manager._runtime.scene_ordering_diagnostic == ("pending_scene_discarded_by_timeout")
    events = _read_events(bridge_root / writer.game_id / "events.jsonl")
    assert all(event["type"] != "scene_changed" for event in events)
...
def test_pending_visual_scene_commits_with_stable_line_when_observed_disabled(
    ...
    assert events[-1]["payload"]["scene_id"] == scene_id
    assert manager._runtime.scene_ordering_diagnostic == ("pending_scene_committed_before_stable")
...
def test_pending_background_scene_is_suppressed_for_narration_continuation(
    ...
    assert scene_events == []
    assert manager._pending_visual_scene_hash == ""
    assert manager._runtime.scene_ordering_diagnostic == ("pending_scene_suppressed_by_dialogue_continuation")
...
def test_pending_background_scene_is_suppressed_for_two_speaker_dialogue(
    ...
    assert scene_events == []
    assert manager._pending_visual_scene_hash == ""
    stable_payloads = [event["payload"] for event in events if event["type"] == "line_changed"]
...
def test_background_hash_distance_20_does_not_advance_visual_scene(
    ...
    assert manager._hash_distance("ff01010101010101", "ff00000010787878") == 20
    assert (
        manager._observe_background_hash(
            "ff00000010787878",
            now=3000.0,
...
def test_configured_background_scene_change_distance_does_not_change_force_threshold(
    ...
    assert manager._pending_visual_scene_hash == ""
    assert manager._runtime.scene_ordering_diagnostic == ("pending_scene_confirmed_by_force_background_distance")
...
def test_followup_background_hash_pending_scene_commits_before_line(
    ...
    assert manager._pending_visual_scene_hash == "ffffffffffffffff"
    assert manager._runtime.scene_ordering_diagnostic == ("followup_background_hash_scene_pending")
    assert not (
        manager._emit_line_from_ocr_text(
            "王生：新场景。",
            now=3000.1,
    ...
    scene_id = events[-2]["payload"]["scene_id"]
    assert events[-1]["payload"]["scene_id"] == scene_id
    assert manager._runtime.scene_ordering_diagnostic == ("followup_background_hash_scene_committed")
...
def test_background_candidate_waits_for_repeat_before_first_new_scene_line(
    ...
    assert manager._runtime.scene_ordering_diagnostic == ("background_candidate_committed_before_observed")
...
def test_background_candidate_is_suppressed_for_short_narration_continuation(
    ...
    assert manager._runtime.scene_ordering_diagnostic == ("background_candidate_suppressed_by_dialogue_continuation")
...
def test_background_candidate_force_distance_still_waits_for_repeat(
    ...
    scene_events = [event for event in events if event["type"] == "scene_changed"]
    assert len(scene_events) == 1
    assert manager._runtime.scene_ordering_diagnostic == ("background_candidate_committed_by_force_distance")
...
def test_compound_neko_ui_capture_is_rejected_without_blocking_single_game_word() -> None:
    polluted = "动画设置 主动搭话 隐私模式 角色设置 API密钥 +++声纹身份 出家门后，算上换乘我一共坐了2个小时的电车。S英"
...
def test_ocr_reader_logs_when_vision_classifier_loads(
    ...
    from market_plugins.galgame_plugin.core import vision as vision_module
    ...
    manager._config = SimpleNamespace(
        ...
        vision_classifier_model_dir=str(
            Path("plugin") / "plugins" / "galgame_plugin" / "models" / "vision" / "screen_classifier"
        ),
...
async def test_ocr_reader_shutdown_waits_for_deferred_vision_initialization(
    ...
    initialization = asyncio.create_task(asyncio.to_thread(manager.initialize_vision_classifier_if_needed))
...
async def test_ocr_reader_rejects_untrusted_primary_capture_before_background_hash(
    ...
    monkeypatch.setattr(
        ...
        lambda background_hash, **_kwargs: (observed_background_hashes.append(background_hash) or True),
    )
...
async def test_ocr_reader_rejects_untrusted_followup_before_stable_line(
    ...
    try:
        ...
        events = _read_events(bridge_root / result.runtime["game_id"] / "events.jsonl")
...
async def test_ocr_reader_discards_new_failed_session_with_existing_history(
    ...
    assert manager._writer.session_id
    assert "session_started" in [event["type"] for event in events[len(events_after_failure) :]]
...
def test_build_config_defaults_ocr_languages_to_chi_sim_jpn_eng(tmp_path: Path) -> None:
    ...
    assert cfg.ocr_reader_bottom_inset_ratio == pytest.approx(DEFAULT_OCR_CAPTURE_BOTTOM_INSET_RATIO)
...
def test_ocr_reader_manager_defers_rapidocr_until_first_ocr(
    ...
    warmup_calls = []
    monkeypatch.setattr(
        "market_plugins.galgame_plugin.ocr_reader.RapidOcrBackend.warmup_async",
...
def test_rapidocr_auto_lang_switches_when_rapidocr_active(
    ...
    manager._rapidocr_backend_cache = SimpleNamespace(close=lambda: backend_close_calls.append(True))
    monkeypatch.setattr(
        galgame_ocr_reader,
        "inspect_rapidocr_installation",
        lambda **_kwargs: {
...
async def test_ocr_reader_manager_prefers_memory_reader_game_window_over_foreground_window(
    ...
    monkeypatch.setattr(
        "market_plugins.galgame_plugin.ocr_reader._foreground_window_handle",
        lambda: 202,
    )
    manager = OcrReaderManager(
        logger=_Logger(),
...
async def test_aihong_menu_stage_returns_to_dialogue_profile_after_stable_line(
    ...
    assert session["state"]["is_menu_open"] is False
    assert any(call[1]["top_ratio"] == pytest.approx(0.0) for call in capture_backend.capture_calls)
    assert latest.runtime["capture_profile"]["top_ratio"] == pytest.approx(0.60)
...
def test_rapidocr_text_adapter_drops_short_low_confidence_ui_marker_line() -> None:
    ...
    assert [text for text, _score, _box in lines] == ["此时此刻，王生正摆了一桌宴席。"]
...
async def test_ocr_reader_manager_auto_mode_prefers_rapidocr_when_available(
    ...
    install_root = ocr_runtime_root
    monkeypatch.setattr(
        "market_plugins.galgame_plugin.ocr_reader.inspect_rapidocr_installation",
        lambda **kwargs: {
            "installed": True,
            "detail": "installed",
            "detected_path": "C:/RapidOCR/site-packages/rapidocr_onnxruntime",
    ...
    monkeypatch.setattr(
        "market_plugins.galgame_plugin.ocr_reader.RapidOcrBackend.extract_text",
...
async def test_ocr_reader_manager_auto_prefers_memory_reader_target_over_stale_manual_target(
    ...
    assert result.runtime["target_selection_detail"] == "manual_target_overridden_by_memory_reader_pid"
...
async def test_ocr_reader_manager_auto_does_not_fall_back_to_stale_manual_when_memory_target_missing(
    ...
    assert result.runtime["status"] == "idle"
    assert result.runtime["detail"] == "waiting_for_valid_window"
    assert result.runtime["target_selection_detail"] == "manual_target_overridden_by_memory_reader_unavailable"
    assert manager.current_window_target()["mode"] == "auto"
    assert manager.current_window_target()["window_key"] == ""
