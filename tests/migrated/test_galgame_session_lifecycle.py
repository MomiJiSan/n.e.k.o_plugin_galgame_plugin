...
import pytest
from market_plugins.galgame_plugin.reader import snapshot_events_boundary
from market_plugins.galgame_plugin.session_lifecycle import (
...
def test_startup_scan_set_does_not_make_invalid_outside_session_current() -> None:
    ...
    assert classify_session_origin(session_id="old-session", **common) == SESSION_ORIGIN_PREEXISTING
    assert classify_session_origin(session_id="outside-session", **common) == SESSION_ORIGIN_PREEXISTING
...
def test_snapshot_events_boundary_pairs_captured_offset_with_seq_high_water(
    ...
    checkpoint_ahead_line = b'{"session_id":"sess-a","seq":2,"type":"line_changed"}\n'
...
def test_snapshot_events_boundary_stops_at_session_checkpoint(tmp_path: Path) -> None:
    ...
    events_path.write_bytes(
        checkpoint_line + b'{"session_id":"sess-a","seq":2}\n' + b'{"session_id":"sess-b","seq":1}\n'
    )
...
def test_snapshot_events_boundary_bounds_checkpoint_scan_to_warmup_window(
    ...
    events_path = tmp_path / "events.jsonl"
    old_prefix = b"".join(
        b'{"session_id":"old","seq":%d,"padding":"' % seq + b"x" * 80 + b'"}\n' for seq in range(1, 101)
    )
...
def test_snapshot_events_boundary_uses_bounded_fallback_when_checkpoint_is_old(
    ...
    new_lines = b'{"session_id":"sess-a","seq":2}\n{"session_id":"sess-a","seq":3}\n'
...
def test_snapshot_events_boundary_keeps_record_at_exact_window_start(
    ...
    events_path = tmp_path / "events.jsonl"
    old_prefix = (
        b'{"session_id":"sess-a","seq":1}\n' + b'{"session_id":"old","seq":1,"padding":"' + b"x" * 512 + b'"}\n'
    )
    new_lines = b'{"session_id":"sess-a","seq":2}\n{"session_id":"sess-a","seq":3}\n'
    events_path.write_bytes(old_prefix + new_lines)
...
def test_snapshot_events_boundary_uses_eof_for_nonpositive_checkpoint(
    ...
    events_path = tmp_path / "events.jsonl"
    complete_stream = b'{"session_id":"sess-a","seq":1}\n{"session_id":"sess-a","seq":2}\n'
