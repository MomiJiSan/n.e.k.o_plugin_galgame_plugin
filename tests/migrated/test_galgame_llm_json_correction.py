...
import pytest
from market_plugins.galgame_plugin.llm_backend import (
    _JSON_CORRECTION_BAD_OUTPUT_MAX_TOKENS,
    GalgameLLMBackend,
)
from market_plugins.galgame_plugin.llm_gateway import LLMGateway
...
async def test_json_correction_prompt_bounds_bad_output_and_succeeds() -> None:
    bad_output = (
        "not json " + ("x" * ((_JSON_CORRECTION_BAD_OUTPUT_MAX_TOKENS * 4) + 32)) + "TAIL_SHOULD_NOT_BE_INCLUDED"
    )
