...
import httpx
import pytest
from market_plugins.galgame_plugin import memory_reader as galgame_memory_reader
from market_plugins.galgame_plugin import textractor_support as galgame_textractor_support
from market_plugins.galgame_plugin.memory_reader import (
    DetectedGameProcess,
...
from market_plugins.galgame_plugin.textractor_support import (
...
def test_memory_reader_ctypes_proxy_updates_win32_job_module(monkeypatch: pytest.MonkeyPatch) -> None:
    from market_plugins.galgame_plugin import _win32_job_objects
...
def test_default_process_scanner_orders_candidates_by_create_time_then_pid(
    ...
    monkeypatch.setattr(
        "market_plugins.galgame_plugin.memory_reader.psutil",
        fake_psutil,
    )
...
async def test_textractor_stdout_reader_logs_regular_exception() -> None:
    ...
    assert any(
        level == "warning" and args[0] == "memory_reader Textractor stdout reader failed: {}"
...
async def test_memory_reader_manager_marks_attached_without_text_when_only_textractor_logs_exist(
    ...
    assert not any(
        level == "debug" and args[0] == "memory_reader Textractor log: {}" for level, args in logger.messages
    )
...
async def test_install_textractor_records_preflight_state_before_request(
    ...
    monkeypatch.setattr("market_plugins.galgame_plugin.textractor_support.update_install_task_state", _record_state)
