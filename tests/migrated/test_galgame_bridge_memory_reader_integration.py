...
galgame_package = import_module("market_plugins.galgame_plugin")
context_builder_module = import_module(
    "market_plugins.galgame_plugin.context_builder"
)
...
GalgameBridgePlugin = plugin_core_module.GalgamePlugin
...
galgame_package.build_summarize_context = context_builder_module.build_summarize_context
...
class _Ctx:
    ...
    def register_static_ui(self, *_args, **_kwargs):
        return None

    def set_list_actions(self, *_args, **_kwargs):
        return None
...
async def test_memory_reader_bridge_tick_fallback_and_bridge_sdk_takeover(tmp_path: Path) -> None:
    ...
    assert isinstance(startup, Ok), str(startup.error)
