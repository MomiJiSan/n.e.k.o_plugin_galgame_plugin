...
import numpy as np
import pytest
from market_plugins.galgame_plugin.training.shared.metrics import macro_f1, top1_accuracy
from PIL import Image


def test_game_screen_cnn_forward_shape() -> None:
    torch = pytest.importorskip("torch")
    from market_plugins.galgame_plugin.training.classify.model import FEATURE_DIM, GameScreenCNN
...
def test_game_screen_dataset_loads_jsonl_and_returns_tensor(tmp_path) -> None:
    ...
    from market_plugins.galgame_plugin.training.data.dataset import GameScreenDataset
...
def test_pretrained_feature_loader_ignores_incompatible_shapes() -> None:
    torch = pytest.importorskip("torch")
    from market_plugins.galgame_plugin.training.classify.model import GameScreenCNN
    from market_plugins.galgame_plugin.training.classify.train import _load_compatible_feature_weights
...
def test_pretrained_backbone_warning_when_torchvision_load_fails(monkeypatch, caplog) -> None:
    pytest.importorskip("torch")
    from market_plugins.galgame_plugin.training.classify import train as train_module
...
def test_train_transform_warns_when_albumentations_fails(monkeypatch, caplog) -> None:
    pytest.importorskip("torch")
    from market_plugins.galgame_plugin.training.shared import augment
...
def test_train_epoch_rejects_non_finite_loss() -> None:
    torch = pytest.importorskip("torch")
    from market_plugins.galgame_plugin.training.classify.train import train_epoch
...
def test_eval_report_creates_parent_directory(tmp_path) -> None:
    from market_plugins.galgame_plugin.training.classify.eval import write_eval_report
...
def test_eval_transform_accepts_grayscale_and_rgba_arrays() -> None:
    torch = pytest.importorskip("torch")
    from market_plugins.galgame_plugin.training.shared.augment import build_eval_transform
...
def test_collect_screenshots_validates_input_dir_before_writing(tmp_path) -> None:
    from market_plugins.galgame_plugin.training.data.collect_screenshots import collect_from_filenames
...
def test_train_validates_num_classes_range() -> None:
    pytest.importorskip("torch")
    from market_plugins.galgame_plugin.training.classify.train import _validate_num_classes
    from market_plugins.galgame_plugin.training.data.dataset import GALGAME_SCREEN_LABELS
